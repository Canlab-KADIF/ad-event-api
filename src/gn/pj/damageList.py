...
PjDamageList = Namespace('PjDamageList')

PjDamageListGet = PjDamageList.schema_model('PjDamageListGet', {

})

@PjDamageList.route('', methods=['GET'])
class PjDamageListApi(Resource):
	####################################################################################################################
	# METHOD : GET
	# write  : 23.08.23
	# writer : chside
	####################################################################################################################
	swagger = PjDamageList.parser()
	...
	@PjDamageList.expect(swagger)

	@PjDamageList.doc(model=PjDamageListGet)
		...
		try:
			...
			if hasParam :
				...
				sql_field = """
					...
					d.lng,
					d.damage_tp,
					(SELECT code_name FROM sy_common WHERE parent_id = '020000' AND code_step = 2 AND code_value =
					d.damage_tp) AS damageNm,
					d.progress_tp,
				...
				if searchText is not None:
					sql_where += "AND d.memo LIKE '%%%s%%' " % searchText
				...
				for idx, row in enumerate(cursor.fetchall()):
					...
					item['beforeImage'] = get_contents_file(row['seq'], tblId, tblTp, svt)
