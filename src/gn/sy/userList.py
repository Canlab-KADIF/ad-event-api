...
SyUserList = Namespace('SyUserList')

SyUserListGet = SyUserList.schema_model('SyUserListGet', {

})

@SyUserList.route('', methods=['GET'])
class SyUserListApi(Resource):

	swagger = SyUserList.parser()
	...
	@SyUserList.expect(swagger)

	@SyUserList.doc(model=SyUserListGet)
	...
	def get(self):
		...
		try:
			...
			if hasParam :
				...
				user_name,
				`role`,
				(SELECT code_name FROM sy_common WHERE parent_id='010000' AND code_step=2 AND code_value=u.`role`) AS roleNm,
				login_yn,
				group_type,
				(SELECT code_name FROM sy_common WHERE parent_id = '020000' AND code_step=2 AND code_value = u.group_type) AS groupTypeNm,
				last_login_dt,
				login_count,
				delete_id """
				...
				if searchText is not None:
					sql_where = sql_where + "AND (user_name LIKE '%%%s%%' or user_id LIKE '%%%s%%') " % (searchText, searchText)
				...
				for idx, row in enumerate(cursor.fetchall()):
					...
					item['userSeq'] = row['seq']
					item['userId'] = row['user_id']
					item['userName'] = row['user_name']
					item['role'] = row['role']
					item['roleNm'] = row['roleNm']
					item['groupType'] = row['group_type']
					item['groupTypeNm'] = row['groupTypeNm']+'회원'
					item['loginYn'] = row['login_yn']
					if row['login_yn'] == 'Y':
						item['loginYnNm'] = '허용'
					else:
						item['loginYnNm'] = '차단'
					...
					if row['last_login_dt'] is not None:
						item['lastLoginDt'] = row['last_login_dt'].strftime("%Y-%m-%d %H:%M")
					else:
						item['lastLoginDt'] = ''
					item['loginCount'] = row['login_count']

					item['deleteId'] = row['delete_id']
					if row['delete_id'] is None:
						item['userState'] = '회원'
					else:
						item['userState'] = '탈퇴'
				...
				# 회원 권한
				roleList = get_common_code_list('010000', '1', svt)
