...
MdCategoryList = Namespace('MdCategoryList')

MdCategoryListGet = MdCategoryList.schema_model('MdCategoryListGet', {
})

@MdCategoryList.route('', methods=['GET'])
class MdCategoryListApi(Resource):
	####################################################################################################################
	# METHOD : GET
	# write  : 23.07.06
	# writer : chside
	####################################################################################################################
	# swagger 파라미터
	swagger = MdCategoryList.parser()
	swagger.add_argument('Authorization', required=True, location='headers', help='로그인토큰')
	@MdCategoryList.expect(swagger)

	@MdCategoryList.doc(model=MdCategoryListGet)
